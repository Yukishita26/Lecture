logLikelifood.3dzx <- function(rho, theta, phi){
  if(rho<0 || theta<0 || theta>pi || phi<0 || phi>2*pi){
    return(-10**9)
  }
  ll <- (6 + 2) * log(rho) - rho*2/3 + log(sin(theta) * (sin(theta) * cos(theta) * cos(phi))**2 + 1e-100)
  return(ll)
}
...
sampling <- function(samples=1000,
  ...
  xyz.vec$y[1] = xyz[2]
  xyz.vec$z[1] = xyz[3]
  last.ll <- logLikelifood.3dzx(last.sample[1], last.sample[2], last.sample[3])
  for(i in 2:samples){
    ...
    new.ll <- logLikelifood.3dzx(new.sample[1], new.sample[2], new.sample[3])
