logLikelifood.3pz <- function(rho, theta, phi){
  phi <- phi %% (2*pi)
  if(theta<0){
    theta <- -theta
    phi <- phi - 2 * pi
  ...
  ll <- log(rho) - rho*2/3 + log((6-rho)**2 * sin(theta) * cos(theta)**2 + 1e-100)
  return(ll)
}
...
sampling <- function(samples=1000,
  ...
  xyz.vec$y[1] = xyz[2]
  xyz.vec$z[1] = xyz[3]
  last.ll <- logLikelifood.3pz(last.sample[1], last.sample[2], last.sample[3])
  for(i in 2:samples){
    ...
    new.ll <- logLikelifood.3pz(new.sample[1], new.sample[2], new.sample[3])
...
sample.size <- 10**5
stepwidth <- 1
samples <- sampling(sample.size, stepwidth = c(stepwidth, stepwidth, stepwidth))
x <- samples$x[seq(1,sample.size,by=1)]
y <- samples$y[seq(1,sample.size,by=1)]
z <- samples$z[seq(1,sample.size,by=1)]
rgl::plot3d(x, y, z, size=1, xlim=c(-20,20), ylim=c(-20,20), zlim=c(-20,20))
