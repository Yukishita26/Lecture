logLikelifood.2pz <- function(rho, theta, phi){
  phi <- phi %% (2*pi)
  if(theta<0){
    theta <- -theta
    phi <- phi - 2 * pi
  ...
  ll <- 3 * log(rho) - rho + log(sin(theta) * cos(theta)**2 + 1e-100)
  return(ll)
}
...
sampling <- function(samples=1000,
  ...
  xyz.vec$y[1] = xyz[2]
  xyz.vec$z[1] = xyz[3]
  last.ll <- logLikelifood.2pz(last.sample[1], last.sample[2], last.sample[3])
  for(i in 2:samples){
    ...
    new.ll <- logLikelifood.2pz(new.sample[1], new.sample[2], new.sample[3])
